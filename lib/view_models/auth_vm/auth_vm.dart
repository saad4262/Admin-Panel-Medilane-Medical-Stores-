...
class AuthController3 extends GetxController {
  ...
  void onInit() {
    ...
    if (firebaseUser != null) {
      ...
      user.value = UserModel(
        ...
        medicalstoreName: firebaseUser.displayName ?? "",
    ...
    _auth.authStateChanges().listen((User? newUser) {
      if (newUser != null) {
        ...
        user.value = UserModel(
          uid: newUser.uid,
          medicalstoreName: newUser.displayName ?? "",
          email: newUser.email ?? "",
  ...
  Future<void> signUp(String username, String email, String password, String confirmPassword, File imageUrl) async {
    if (password != confirmPassword) {
      Get.snackbar("Error", "Passwords do not match!", backgroundColor: AppColor.redColor);
      return;
    }
    isLoading.value = true;
    UserModel? newUser = await _authRepository.signUp(username, email, password,imageUrl);
    if (newUser != null) {
      user.value = newUser;

      // ✅ Save user data in Firestore
      await _firestore.collection("medicalstores").doc(newUser.uid).set(newUser.toJson());
  ...
  Future<void> uploadImageToFirebase() async {
    ...
    try {
      final AuthController3 authController = Get.find<AuthController3>(); // Get the AuthController instance

      final storageRef = FirebaseStorage.instance
          .ref()
          .child("medicalStore_profile_images/${authController.user.value!.uid}.jpg");
      ...
      await FirebaseFirestore.instance
          ...
          .update({'medicalStores_profileImage': downloadURL});
