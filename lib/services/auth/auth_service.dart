...
//
//       UserModel newUser = UserModel(uid: uid, username: username, email: email, medicalstoreName: '',);
//
...
class FirebaseAuthService {
  ...
  // ✅ Signup Method
  Future<UserModel?> signUp(String username, String email, String password, File imageFile) async {
    try {
      UserCredential userCredential = await _auth.createUserWithEmailAndPassword(
        email: email,
        password: password,
      ...
      String uid = userCredential.user!.uid;
      String imageUrl = await uploadProfilePic(uid, imageFile); // Call uploadProfilePic to get the image URL


      UserModel newUser = UserModel(uid: uid, medicalstoreName: username, email: email,profilePic: imageUrl);
